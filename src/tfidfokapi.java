...
public class tfidfokapi {
	...
	public static void main(String[] args) throws IOException {
		...
		while(queritr.hasNext())
		{
			...
			for(int i = 0; i < words.length ; i++)
			{
				...
				resultmap =  tfidf(local, resultmap, idmap, lengthmap, catalogmap);
			}
	...
	private static HashMap<String, Double> tfidf(String local,HashMap<String, Double> resultmap,
			...
			HashMap<String, String> catalogmap) throws IOException {
		...
		if (catalogmap.containsKey(local))
		{
			...
			for (int i = 1; i < al.length ; i++)
			{
				...
				{
					double oldtf = resultmap.get(doc);
					resultmap.put(doc, oldtf + okapitf);
				}
				else
					resultmap.put(doc, okapitf);
			}
	...
	private static void printmap(HashMap<String, Double> resultmap, String string) {
		...
		int id0 = 0;
		File log = new File("C:\\Users\\AKI\\workspace\\Ass2\\tfidf.txt");
